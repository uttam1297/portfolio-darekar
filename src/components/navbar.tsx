...
export function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 h-[52px] bg-[#0f0f12]/80 backdrop-blur-xl border-b border-border">
      <div className="mx-auto max-w-[960px] h-full flex items-center justify-between px-5">
        <a
          href="#top"
          className="font-extrabold text-zinc-200 tracking-wide"
        >
          UD
        </a>
      ...
      {open && (
        <nav className="sm:hidden bg-[#0f0f12]/97 backdrop-blur-2xl border-b border-border px-5 py-3 flex flex-col gap-0.5">
          {links.map((l) => (
            <a
              key={l.href}
              href={l.href}
